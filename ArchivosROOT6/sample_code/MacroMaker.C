...
void mkexe_with_flag(char *, char *, int);        //YO LO CAMBIE, SE PUSO EL PROTOTIPO
void mklib_with_flag(char *,int);		  //YO LO CAMBIE, ........
...
//executables
  ...
  int result;
  char pwd[4]="./";
  const char *libs;                           //Yo LO CAMBIE, tocaba poner un *const*
  string myMakeSharedLibString;               //YO LO CAMBIE esta char myMakeSharedLibString[500];
  if(debug){
    myMakeSharedLibString="cd $BuildDir ; g++ -c $Opt -pipe -Wno-deprecated -Wno-unused -Wall -Woverloaded-virtual -fPIC -Iinclude -D_REENTRANT $IncludePath $SourceFiles ; g++ $ObjectFiles -shared -Wl,-soname,$LibName.so -g -o $SharedLib";
  }
  else{
    myMakeSharedLibString="cd $BuildDir ; g++ -c $Opt -pipe -Wno-deprecated -Wno-unused -Wall -Woverloaded-virtual -fPIC -Iinclude -D_REENTRANT $IncludePath $SourceFiles ; g++ $ObjectFiles -shared -Wl,-soname,$LibName.so -O -o $SharedLib";
  }
	
  const char *makeSharedLibString;           //YO LO CAMBIE, tocaba poner un *const*
  ...
  makeSharedLibString=gSystem->GetMakeSharedLib();
  gSystem->SetMakeSharedLib(myMakeSharedLibString.c_str());
...
//-lRootReer and -lPhysics are already in.
  ...
  if(debug){
  sprintf(compileString,"g++ -g -o %s -Wno-deprecated -Wall -Wno-unused -Woverloaded-virtual -fPIC %s -D_REENTRANT -DROOTEXE %s -lPhysics -lThread -lm -ldl -lCore -lCling -lRIO -lNet -lHist -lGraf -lGraf3d -lGpad -lTree -lRint -lPostscript -lMatrix -lPhysics -lMathCore -lThread -lGui -pthread -lm -ldl -rdynamic  -pthread -traditional -L$ROOTBEER_SLIB -lRootBeer %s %s",target, gSystem->GetIncludePath(),gSystem->GetLinkedLibs(),extra,filename);    //YO LO CAMBIE, se cambia -lCint al compilador de ROOT6 -lCling
  }
  else{
  sprintf(compileString,"g++ -O2 -o %s -Wno-deprecated -Wall -Wno-unused -Woverloaded-virtual -fPIC %s -D_REENTRANT -DROOTEXE %s -lPhysics -lThread -lm -ldl -lCore -lCling -lRIO -lNet -lHist -lGraf -lGraf3d -lGpad -lTree -lRint -lPostscript -lMatrix -lPhysics -lMathCore -lThread -lGui -pthread -lm -ldl -rdynamic -pthread -traditional -L$ROOTBEER_SLIB -lRootBeer %s %s",target, gSystem->GetIncludePath(),gSystem->GetLinkedLibs(),extra,filename);      //YO LO CAMBIA, se cambia -lCint ....
  }
